...
class CustomForm extends React.Component{
    ...
    handleFormSubmit = (event, requestType, artifactID) => {
        ...
        switch(requestType){
            ...
            case 'put':
                return axios.put('http://127.0.0.1:8000/api/'+ artifactID + '/', form_data, {
                    headers: {
                      'content-type': 'multipart/form-data'
                    }
                }).then(res=>console.log(res)).catch(error=>console.error(error));
    ...
    render(){
        return (
            <div>
                <Form onSubmitCapture={(event) => this.handleSubmit(event, this.props.requestType, this.props.artifactID)}>
